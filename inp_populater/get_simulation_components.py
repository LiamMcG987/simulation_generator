...
from .extract_component_instances import InstanceInp
...
class PopulateSimulation:
    ...
    def __init__(self, sbm_data, bottle: str, simulation: str, bottle_mass: float, translation: list[int]):
        """
    ...
    def _write_simulation_inp(self):
        ...
        for file in self.inp_files:
            ...
            print(file_name)
            simulation_inp_components[file_name] = InstanceInp(file, self.simulation, self.sbm_data, self.bottle_mass)

        self.parts_dict = simulation_inp_components
        WriteToInp(self.parts_dict, self.template_file_path, simulation_inp, self.translation)

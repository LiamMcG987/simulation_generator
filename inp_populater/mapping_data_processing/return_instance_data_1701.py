...
class Instance:
    ...
    def __init__(self, idx_pair, lines, file_name, abq_file=None):
        self.file_name = file_name
        self.start_idx = idx_pair[0]
        self.lines = lines
        self.abq_file = abq_file if abq_file is not None else True
    ...
    def three_dx_check(self, lines):
        """ Checks if file is 3dx file - parses first few lines. """
        node_idx = 0
    ...
    def extract_coords(self, lines):
        ...
        self.three_dx_check(lines)
        if not self.abq_file:
            return self.extract_coords_three_dx(lines, self.file_name)
    ...
    def extract_coords_three_dx(self, lines, inp_deck_name):
        """ Extracts data from 3dx, parses through to return only bottle nodes."""
        ...
        if 'bulge' in inp_deck_name.lower():
